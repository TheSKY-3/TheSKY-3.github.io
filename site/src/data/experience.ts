...
export const experience = {
  ...
  highlights: [
    ...
    {
      title: "出租车三方结算口径标准化与自动化",
      problem:
        "司机、车企、平台三方结算长期靠人工经验判断：一笔钱该计入哪项、算在哪一天、是否参与计佣，没有一份研发能直接照着实现的文档。",
      actions: [
        "把口口相传的扣费与调整规则逐条追问来源，改写成可判定条款——每条都要能回答「满足时怎么算、不满足时怎么算」",
        "设计 21 项标准结算字段、边界规则与可复算的验证案例，作为研发实现与业务复核共用的唯一口径",
        "固化为本地计算程序：输入对账明细按司机×日期输出结果；字段缺失或主键重复时中止，而不是补默认值硬算",
      ],
      outcome:
        "研发实现与业务复核第一次对齐到同一份口径；部门实测：同类结算复核从约 3 小时缩短到约 5 分钟。",
      tags: ["口径标准化", "边界规则", "研发协同"],
      metric: { value: "3h→5min", label: "结算复核（部门实测）" },
    },
    {
      title: "财务对账自动化",
    ...
    {
      title: "校企运营优化方向先期诊断与对接",
      problem:
        "先期经营诊断有分析价值，但只停留在部门内部——没有进入能决定资源投向的场合。",
      actions: [
        "促成企业与高校统计学院就运营优化方向的首次工作对接",
        "会上不讲结论，讲问题拆解：「打不到车」拆成供需、司机行为、区域、时段、渠道、个体六层分别定位",
        "同一套分析按经营决策 / 工程对接 / 学术方法三种口径重组后汇报",
      ],
      outcome: "对接完成，运营优化方向进入双方商谈（尚未立项开发）。",
      tags: ["问题拆解", "跨方对接", "管理层汇报"],
    },
