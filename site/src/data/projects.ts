...
export const projects: Project[] = [
  ...
  },
  {
    ...
    figures: [
      ...
      { src: "/images/ecom/workbench-diagnosis.png", alt: "SKU 诊断工作流实测截图", caption: "SKU 诊断实测（受控别名 A · 运行噪音）：返回已校验证据卡（支持 / 信息不足）与人工核查建议；评论原文默认私密，点开单卡才按需返回" },
      { src: "/images/ecom/workbench-comparison.png", alt: "竞品比较工作流实测截图", caption: "竞品比较实测（受控别名 A vs B · 制冰质量）：信号表只陈述证据方向，不替运营宣布谁更好" },
    ...
    sections: [
      ...
      },
      {
        ...
        body: "① 治理层：冻结公开评论数据快照、体验维度与证据合同——口径先于代码，分析不能临时创造标签。② 证据层：将评论装入 SQLite/FTS5 本地引擎，让确定性代码而不是模型掌握事实；模型只拿运行期别名和聚合证据卡，编造与串商品在结构上受约束。③ 智能层：LangGraph 受约束 Agent 只做澄清与规划，本地 Verifier 逐字段核对极性、强度、引用并回库校验。④ 业务层：工作台把能力变成三个任务流，运营看到的是可回查的证据卡，而不是一段无法复核的总结。",
      },
      {
        heading: "三大工作流（本地受控验证）",
        body: "界面上商品以受控别名出现——这是刻意设计：运行时不泄露内部商品标识，评论原文默认私密、点开单卡才按需返回。三个工作流分别输出已校验证据卡、竞品信号表和问题验证顺序；冲突证据会显式标出并附责任团队。",
      },
      {
        heading: "为什么可信：三道防编造设计 + 一道评测门",
        body: "第一道，口径冻结——防「模型或开发者临时创造标签」；第二道，本地引擎——防编造证据、防把别的商品评论算到当前商品头上；第三道，Verifier 回库校验——防模型改写证据卡的极性和强度。评测门由冻结案例阻断测试和独立模型结果组成，安全指标来自本地执行记录与集合比较，模型「说自己没越权」不算数。",
      },
      {
        heading: "两个值得讲的工程取舍",
        body: "① 检索负结果：冻结案例对比后 FTS5/BM25 没有显示独立优势，项目没有为了「技术栈好看」强行上向量数据库——同时记录该结论的循环偏向边界，若建立独立人工相关性集需重新比较。技术选型服务于问题，而不是简历。② 分层评测：检索、Agent 离线、模型评测、工作台各有独立 Gate 和「能证明什么/不能证明什么」清单——防止一个绿色对勾掩盖所有问题。",
      },
      {
        heading: "边界与状态",
        body: "工程门已按项目手册完成阶段性验证，工作台处于本地受控验证状态。它不根据评论推断真实缺陷率、销量或利润，也不回答「该买哪个」；工程验证不能代替真人试用——这是项目刻意保持的状态标注。",
      },
    ],
    boundary:
