...
- 业务数字只允许使用简历已对外表述的口径：1015 万条订单、约 40 名司机、60 天/60 期、2 小时→5 分钟、近 20 份报告、21 项结算字段、约 159 万条风控样本、40.37%-48.60% 成本下降、10% 复核容量、约 1398 万行（13,979,592）Criteo 数据、212 万条评论（2,128,605）、210 万条精确关联（2,100,939）、3,278 个商品、136,604 条评论、99.87% 文本覆盖率。
...
export const experience = {
  ...
  items: [
    ...
    { title: "出租车三方结算口径标准化", detail: "梳理司机、车企与平台三方资金结算链路，把口口相传的扣费与调整规则改写为可判定条款；设计 21 项标准结算字段、边界规则与验证案例，协同研发推进系统实现，支持业务复核与审计追溯。" },
  ] as ExperienceItem[],
};
```
